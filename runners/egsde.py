...
class EGSDE(object):
    ...
    def egsde(self,):
        ...
        if args.diffusionmodel == 'ADM':
            ...
            states = torch.load(args.ckpt, map_location=torch.device('cpu'))
            ...
            model = Model(config)
            states=torch.load(self.args.ckpt, map_location=torch.device('cpu'))
        ...
        dse = create_dse(image_size=config.data.image_size,
                         ...
                         phase=args.phase)
        states = torch.load(args.dsepath, map_location=torch.device('cpu'))
